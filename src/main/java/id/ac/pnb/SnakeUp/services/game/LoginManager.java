...
public ModelUser login(ModelLogin login) {
    ...
    public void updateGame(int userID) {
    try {
        int matchResult =1;
        if (con != null) {
            String query = "UPDATE `winrate` SET `match` = ? WHERE `id_user` = ?";
            try (PreparedStatement p = con.prepareStatement(query)) {
                p.setInt(1, matchResult);
                p.setInt(2, userID);
                p.executeUpdate();
            }
        } else {
    ...
    public void updateWinrate(int userID) {
    try {
        int gameWin =1;
        if (con != null) {
            String query = "UPDATE `winrate` SET `gameWin` = ? WHERE `id_user` = ?";
            try (PreparedStatement p = con.prepareStatement(query)) {
